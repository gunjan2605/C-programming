...
//declare function
void generate_people(People* passenger,People* passenger_e1, People* passenger_e2,People* passenger_e3, int total_count, int* total_count_e1, int* total_count_e2,int* total_count_e3, int maxFloor1, int maxFloor2);
void elevator_run(int n, int maxFloor1, int maxFloor2);
...
int main(){
    ...
    {
        int maxFloor1 = r;
        for (int s = 11; s<18; s+=2)
        {
            int maxFloor2 = s;
            elevator_run(1000, maxFloor1, maxFloor2);
            printf("%d, %d \n", maxFloor1,maxFloor2);
...
void elevator_run(int n, int maxFloor1,int maxFloor2)
{
    ...
    for (int t = 0; t<n; t++)
    {
        ...
        People passenger_e3[total_count]; //declare an array of type struct people for elevator 3

        generate_people(passenger, passenger_e1, passenger_e2, passenger_e3, total_count, &total_count_e1, &total_count_e2, &total_count_e3, maxFloor1, maxFloor2); 
        //generate passenger list containing exit floor, travel time and wait time
        ...
        if(total_count_e3>0){
            c = passenger_e3[total_count_e3-1].waitTime + passenger_e3[total_count_e3 -1].travelTime; //total time for elevator 3
        }
        ...
        for (i = 0;i<total_count_e3; i++)
        { 
            sum = sum + passenger_e3[i].waitTime + passenger_e3[i].travelTime;
        } 
...
{   
    ...
    for(i = 0; i<total_count; i++)
    {
        ...
        (passenger+i)->elevatorNumber = 1*((passenger+i)->ExitFloor <= maxFloor1) + 2*(((passenger+i)->ExitFloor > maxFloor1) && ((passenger+i)->ExitFloor <= maxFloor2)) + 3*((passenger+i)->ExitFloor > maxFloor2);
    ...
    for(i=0; i<total_count; i++)
    {
        ...
        if(passenger[i].elevatorNumber == 2)
        {
        ...
        {
            *(passenger_e3 + *(total_count_e3)) = passenger[i];
            (*total_count_e3)++; 
        }
    }
    ...
    sortarray(passenger_e3, total_count_e3);

    caltime(passenger_e1, total_count_e1);
    caltime(passenger_e2, total_count_e2);
    caltime(passenger_e3, total_count_e3);
